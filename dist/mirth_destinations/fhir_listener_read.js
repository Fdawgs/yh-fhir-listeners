...
try {
  ...
  switch ("".concat(type)) {
    case 'allergyintolerance':
      wherePredicate = ["(REPLACE(alle.ALG_RowId, ''||'', ''-'') = ''".concat(id, "'')")];
      break;
    ...
    case 'encounter':
      wherePredicate = ["(REPLACE(app.APPT_RowId, ''||'', ''-'') = ''".concat(id, "'')"), "(REPLACE(PAADM_ADMNo, ''/'', ''-'') = ''".concat(id, "'')"), "(REPLACE(TRANS_ParRef->PAADM_ADMNo, ''/'', ''-'') = ''".concat(id, "'')")];
      break;

    case 'medicationstatement':
      wherePredicate = ["(REPLACE(oi.OEORI_RowID, ''||'', ''-'') = ''".concat(id, "'')"), ''];
      break;
  ...
  if (result.next()) {
    ...
    var response = FhirResponseFactory.getReadResponse(JSON.stringify(data), version, lastModified, 200, 'application/fhir+json');
    responseMap.put('response', response);
    return response.getMessage();
  }

  return createOperationOutcome('error', 'processing', "".concat($('fhirType'), " ID ").concat(id, " not found."), 404);
} catch (error) {
